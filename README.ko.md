...
Switchboard는 Windows 11에서 창 전환을 더 선명하고 안정적으로 만드는 데스크톱 오버레이입니다. 열려 있는 창을 읽기 쉬운 제목과 잘리지 않는 미리보기로 표시하고, 창 개수와 화면 공간에 맞춰 레이아웃을 조절합니다.

**현재 버전:** [v0.3.1](https://github.com/ai-blink/alt-tab/releases/tag/v0.3.1) · **배포 형식:** Windows 11 x64 Portable

> 설치 프로그램 없이 압축을 풀어 실행하는 포터블 프리뷰입니다. 코드 서명이 적용되지 않은 빌드에서는 Windows SmartScreen 경고가 표시될 수 있습니다.

## 주요 기능

- Alt+Tab 한 번으로 Switchboard 표시/숨김 전환
- 원본 창 전체가 보이는 DWM 미리보기
- 25개를 넘는 창도 표시하는 반응형 행·열 및 스크롤
- 격자, 압축, 목록 보기
- 빈 공간 드래그로 저장되는 오버레이 위치, 9방향 일회성 위치 리모컨, 저장 위치 복귀
- 최근 사용, 앱, 모니터, 제목, 즐겨찾기 정렬
- 투명·어두움·밝음 테마, 투명도, 전체 UI 배율(60/70/80/100/125/150/200%), 썸네일 크기 설정
- 사용자 지정 보조 단축키(기본값: Ctrl+Alt+Space)
- 항상 위에 표시 설정, 시스템 트레이 상주, 창 전환을 알리는 전용 앱 아이콘
- 각 창 카드에서 대상 창에 정상 종료를 요청하는 닫기 버튼
- 창 목록이 바뀐 경우에만 갱신해 폴링 깜빡임 최소화

## 설치 및 실행

1. Switchboard-v0.3.1-win-x64-Portable.zip의 압축을 새 폴더에 풉니다.
2. Switchboard.App.exe를 실행합니다.
...
상단 설정 버튼은 별도 모달 창을 엽니다. 왼쪽 탭에서 위치와 이동, 모양과 크기, 기본 동작을 나누어 설정할 수 있습니다. 오버레이의 빈 공간을 드래그하면 위치가 자동 저장되며, 위치 리모컨은 저장 위치를 바꾸지 않고 한 번만 이동합니다. 전체 배율은 글씨·버튼·여백·창 크기를 함께 조절하고, 압축 모드에서는 별도의 추가 축소 배율을 적용하지 않습니다. 핀 버튼은 **항상 위에 표시**를 켜거나 끕니다.

창 닫기 버튼은 강제 종료가 아닌 Windows 표준 종료 요청을 사용합니다. 저장하지 않은 작업이 있으면 대상 앱의 저장 확인 창이 표시될 수 있습니다.
...
기본값으로 초기화하려면 Switchboard를 종료하고 위 파일을 삭제한 뒤 다시 실행하세요.

## 알려진 제한 사항

- 관리자 권한 창과 보안 데스크톱의 창은 Windows 포커스 제한 때문에 활성화에 실패할 수 있습니다.
- 현재 배포 파일에는 디지털 코드 서명이 없어 SmartScreen 경고가 표시될 수 있습니다.
- 현재는 Alt+Tab 훅을 끄는 별도 설정이 없습니다. 기본 Windows Alt+Tab을 사용하려면 Switchboard를 종료해야 합니다.
...
| [계획](notes/plans/) · [실행 기록](notes/runs/) · [Stitch 참고자료](references/stitch/) | 과거 계획과 검증 근거이며, 현재 상태의 정본은 아님 |
