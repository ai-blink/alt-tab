...
- Current behavior: compact window size controls the work-area cap (45%/55%/65%/75%/85%), while UI size controls only the overlay render scale (85%/92.5%/100%/110%/120%). Defaults are `보통` window size and `크게` UI size.
- Completed: custom Switchboard icon now appears in the tray and taskbar window; the Release app was launched successfully.
- Verification: Release build passes with 0 warnings/errors; 20 tests pass; focused compact layout/settings tests pass; HTML mock JavaScript syntax and control counts pass.
